...
def _build_view() -> tuple[SimpleNamespace, list[_Hero], _Building]:
...
def _summaries_for(seed: int = 7) -> list[tuple[str, str, str, str, tuple[float, float] | None]]:
    daily_life.reset_ambient_memory()
    ai = _FixedAI(seed)
    view, heroes, castle = _build_view()

    summaries: list[tuple[str, str, str, str, tuple[float, float] | None]] = []
    for hero in heroes:
...
def test_daily_life_is_deterministic_for_fixed_10_hero_setup(monkeypatch) -> None:
...
def test_daily_life_splits_ten_heroes_across_richer_motives_and_clusters(monkeypatch) -> None:
    monkeypatch.setattr(daily_life, "sim_now_ms", lambda: 20_000)
    summaries = _summaries_for(seed=17)
    motives = Counter(motive for _hero_id, motive, _target_key, _cluster_key, _target_xy in summaries)
    pairs = {(motive, target_key) for _hero_id, motive, target_key, _cluster_key, _target_xy in summaries}
    clusters = {cluster_key for _hero_id, _motive, _target_key, cluster_key, _target_xy in summaries}

    assert len(motives) >= 6
    assert len(pairs) >= 6
    ...
    castle = _build_view()[2]
    far_targets = [
        ((target_xy[0] - castle.center_x) ** 2 + (target_xy[1] - castle.center_y) ** 2) ** 0.5 / TILE_SIZE
        for _hero_id, _motive, _target_key, _cluster_key, target_xy in summaries
