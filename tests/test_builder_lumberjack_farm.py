...
from game.entities.neutral_buildings import Farm
...
def test_builder_requires_multiple_harvests_before_farm_builds() -> None:
    world = _mk_world()
    # Two eligible trees yield 10+10 = 20, enough for a farm.
    world.set_tile(12, 10, int(TileType.TREE))
    world.set_tile(13, 10, int(TileType.TREE))
    sim = _FakeSim(world, trees=[_TreeRec(12, 10, 1.0), _TreeRec(13, 10, 1.0)])

    castle = Building(10, 10, "castle")
    plot = Farm(15, 10, is_constructed=False)
    ...
    # After the first harvest, builder should have 10 wood but farm must still be unconstructed.
    for _ in range(400):
        bp.update(0.1, gs)
    ...
    # After the second harvest, builder should reach 20 and then proceed to build.
    for _ in range(600):
        bp.update(0.1, gs)
        if bp.wood_inventory >= 20:
            break
    assert bp.wood_inventory >= 20
    assert bp.phase in {
