"""WK142 rescue/revenge visual readability pins.

These tests confirm the existing WK139/WK141 boss and elite presentation
language already makes the Blackbanner rescue/revenge targets readable.
No new renderer markers are needed for this wave.
"""
...
def _render_rusk_frame(
    ...
    *,
    ...
    current_phase_title: str = "Avenge Mira",
) -> pygame.Surface:
...
def test_wk142_revenge_target_uses_existing_boss_crown_name_and_telegraph_language():
    registry = RendererRegistry()
    vfx = VFXSystem()

    plain = _render_rusk_frame(registry, vfx, show_boss_marker=False, telegraph_state="none")
    marked = _render_rusk_frame(
        registry,
        vfx,
        show_boss_marker=True,
    ...
    assert _count_match(marked, telegraph_region, _is_orange) > 0
    assert _count_match(marked, telegraph_region, _is_orange) > _count_match(cleared, telegraph_region, _is_orange)


def test_wk142_rescue_target_uses_existing_elite_badge_language():
    registry = RendererRegistry()
