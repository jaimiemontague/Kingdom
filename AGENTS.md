...
- **Version bumps require explicit human authorization**: do not change `config.py` version stamps, README version text, `CHANGELOG.md` release headers, git tags, branch names, or commit messages to imply a new `Prototype vX.Y.Z` unless Jaimie explicitly approves that exact version bump.
- **Git save states use WK sprint names**: sprint commits/pushes should be named like `wk142: Dynamic Rescue & Revenge`, not `Prototype v1.6.x`. Do not rewrite previously pushed version-like commit messages without explicit history-rewrite authorization from Jaimie.
