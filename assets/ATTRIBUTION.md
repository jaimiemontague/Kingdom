...
- **File locations**:
  - `assets/models/environment/lair.glb` (lair building mesh; WK33 copy of `Models/GLB format/crypt-large-graveyard.glb`)
...
- **Retrieved**: 2026-01-27
- **Modifications**: Curated subset (ranged weapon sounds, building destruction, bounty placement, combat sounds, celebration sounds, enemy-type-specific sounds, dragon boss cues); normalized volume levels
- **Used for**: Ranged weapon attack sounds (bow release), building destruction, bounty placement/claim, combat sounds (melee hit, enemy death), celebration sounds (lair cleared), hero hire/purchase confirmations, enemy-type-specific attack/death/ambient sounds (WK61), dragon boss cues for Ashwing's Hoard (WK143)
- **File locations**:
  ...
  - `assets/audio/sfx/dragon_roar.ogg` (WK143: from `weird_04.ogg`)
  - `assets/audio/sfx/dragon_phase.ogg` (WK143: from `gong_02.ogg`)
  - `assets/audio/sfx/dragon_fire_telegraph.ogg` (WK143: from `machine_03.ogg`)
  - `assets/audio/sfx/dragon_fire_impact.ogg` (WK143: from `explosion.ogg`)
...
- **Retrieved**: 2026-04-30
- **Modifications**: Horizontal strips split to per-frame PNGs; black background keyed to alpha; **nearest-neighbor** letterbox into `UNIT_SPRITE_PIXELS` (default `--content-scale` 1.0 preserves vendor sharpness). Builder variant: straw-hat pixels only recolored to green via `tools/legacy_vania_export_worker_frames.py`. Peasant walk: if `npc-villager1-fisherman_walk.png` is a tiny stub (~600 bytes), exporter uses `npc-villager10-anim-walk_strip6.png`. Tax collector uses **npc-cape2** strips without knife walk/attack (idle stance, walk + walk-stance, jab for collect, hurt/fall/recover, waiting crouch); run `python tools/legacy_vania_export_worker_frames.py --execute`. Ursina on-screen worker height uses `URSINA_WORKER_BILLBOARD_BASE`, `URSINA_WORKER_BILLBOARD_Y_SCALE_MUL`, and env overrides (`KINGDOM_URSINA_WORKER_SCALE`, `KINGDOM_URSINA_WORKER_Y_MUL`) in `config.py` — not by downsampling these PNGs.
