...
last_round['pm_status_summary']['status'] = 'closed_success'
last_round['pm_status_summary']['summary'] += ' (Skipped human retest to address urgent peasant pathing bug)'

# Create WK51
wk51_id = 'wk51_peasant_void_walk_hotfix'
data['sprints'][wk51_id] = {
    "sprint_meta": {
        "sprint_id": wk51_id,
        "goal": "Fix peasant pathing bug where they run to the top left of the map after starting work on a building.",
        "plan_ref": "None (Hotfix)"
    },
    "rounds": {
        "wk51_r1_kickoff": {
            "round_meta": {
                "round_id": "wk51_r1_kickoff",
                "created_utc": "2026-05-01T09:55:00Z",
                "source": "pm_kickoff",
                "closed_by": None
            },
            "pm_status_summary": {
            ...
            "pm_agent_prompts": {
                "05": "You are Agent 05, sprint wk51_peasant_void_walk_hotfix, round wk51_r1_kickoff (HIGH). Human playtest bug: peasants go to start working on the first placed building, but then run off towards the top left of the map (likely 0,0). Investigate `game/entities/peasant.py` and related building/economy logic. Fix the pathing/state bug so they stay and build. Run `python tools/qa_smoke.py --quick`. Update your log, validate JSON, complete.",
                "11": "You are Agent 11, sprint wk51_peasant_void_walk_hotfix, round wk51_r1_kickoff (MEDIUM). After Agent 05 fixes the peasant void walk bug, verify the fix. Run `python tools/qa_smoke.py --quick` and ensure the base scenario (which includes construction) passes without peasants running to 0,0. Update your log, validate JSON, complete."
            },
            ...
            "pm_universal_prompt": "You are activated for sprint wk51_peasant_void_walk_hotfix, round wk51_r1_kickoff. We are fixing a blocker bug where peasants run to the top left of the map after starting construction. Update your log, run gates, validate JSON, complete."
        }
    }
}
...
print("Successfully updated PM hub.")
