...
class HUD:
    ...
    def _render_pin_button(self, surface: pygame.Surface, left_rect: pygame.Rect, game_state: dict) -> None:
        ...
        if not hasattr(self, "_pin_emoji_font") or getattr(self, "_pin_emoji_font_size", 0) != pin_size:
            try:
                self._pin_emoji_font = pygame.font.SysFont(
                    "segoeuiemoji,segoeui,noto color emoji,arial",
                    pin_size,
                )
            except Exception:
        ...
        use_emoji = emoji_surf is not None and emoji_surf.get_width() > 4
        if use_emoji:
            ew, eh = emoji_surf.get_width(), emoji_surf.get_height()
            dest_pos = (pr.centerx - ew // 2, pr.centery - eh // 2)
            if pinned:
                surface.blit(emoji_surf, dest_pos)
            else:
                faded = emoji_surf.copy()
                faded.set_alpha(128)
                surface.blit(faded, dest_pos)
        else:
            if pinned:
