"""WK20 — Ursina input debug telemetry.

WK67 Round A-2 (L9): moved verbatim from ``tools/ursina_input_debug.py`` into
``game/graphics/`` so a packaged/frozen build (which cannot ship the dev ``tools/``
tree) can still import this module-top dependency of ``game/graphics/ursina_app.py``.
``tools/ursina_input_debug.py`` now re-exports these symbols from here.
