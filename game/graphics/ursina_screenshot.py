...
WK67 Round A-2 (L9): moved verbatim from ``tools/ursina_screenshot.py`` into
``game/graphics/`` so a packaged/frozen build (which cannot ship the dev ``tools/``
tree) can still import these module-top + F12 dependencies of
``game/graphics/ursina_app.py``. ``tools/ursina_screenshot.py`` now re-exports these
symbols from here. The only adjustment vs. the original is the repo-root walk in
``docs_screenshots_dir`` (this file is one directory deeper than the old tools home).
...
def docs_screenshots_dir() -> str:
    ...
    # This file lives at game/graphics/ursina_screenshot.py -> three dirnames up = repo root.
